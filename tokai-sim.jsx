...
// ------------------------------------------------------------------
// 東海ステークス(G3) 想定隊列データ（中京ダート1400m・左回り）
// ------------------------------------------------------------------
// 総合評価は「東海ステークス(G3) 追い切り」実データ（S>A>B>C>D）
const HORSES = [
  { no: 14, name: "マテンロウコマンド", waku: 8, style: "逃げ", color: "#e8558d", text: "#fff", grade: "B", memo: "最後までしっかりとした脚取り" },
  { no: 13, name: "ドラゴンテイラー",   waku: 8, style: "番手", color: "#e8558d", text: "#fff", grade: "B", memo: "最後までしっかりとした脚取り" },
  { no: 5,  name: "ドンインザムード",   waku: 4, style: "先行", color: "#1c3f8f", text: "#fff", grade: "A", memo: "力強く回転力あり最後までスピード感ある走り" },
  { no: 11, name: "メイショウハチロー", waku: 7, style: "先行", color: "#e8830f", text: "#fff", grade: "A", memo: "跳びが大きく回転力ある走り" },
  { no: 3,  name: "ハッピーマン",       waku: 3, style: "好位", color: "#d9432b", text: "#fff", grade: "B", memo: "遅れはしたが安定感あり余裕のある走り" },
  { no: 7,  name: "ダノンフィーゴ",     waku: 5, style: "好位", color: "#e8b800", text: "#222", grade: "A", memo: "先週は終いでバテたが今週は最後までしっかりとした脚取り" },
  { no: 8,  name: "ベルジュロネット",   waku: 5, style: "好位", color: "#e8b800", text: "#222", grade: "S", memo: "力強い脚捌きで終いもしっかりと伸びる" },
  { no: 9,  name: "インユアパレス",     waku: 6, style: "好位", color: "#2f7a3c", text: "#fff", grade: "C", memo: "やや動き重たく" },
  { no: 4,  name: "モンドプリューム",   waku: 3, style: "中団", color: "#d9432b", text: "#fff", grade: "D", memo: "終始折り合い欠き" },
  { no: 6,  name: "ケイアイドリー",     waku: 4, style: "中団", color: "#1c3f8f", text: "#fff", grade: "B", memo: "集中力には欠けたが終いの伸び良く" },
  { no: 2,  name: "ジュンウィンダム",   waku: 2, style: "中団", color: "#161616", text: "#fff", grade: "C", memo: "集中力かけたが立て直し" },
  { no: 1,  name: "ノーブルロジャー",   waku: 1, style: "後方", color: "#f4f4f2", text: "#222", grade: "B", memo: "最終軽めの調教だが軽快なフットワーク" },
  { no: 10, name: "トリリオンボーイ",   waku: 6, style: "後方", color: "#2f7a3c", text: "#fff", grade: "-", memo: "調教映像なし。最終は併入も中間は遅れ続く" },
  { no: 12, name: "バトルクライ",       waku: 7, style: "後方", color: "#e8830f", text: "#fff", grade: "C", memo: "終いの伸びいまひとつ" },
];
...
// 雨天時の道悪評価材料：東海ステークスは今回データなしのため空。雨モードでは「参考情報なし」と表示される。
const RAIN_NOTES = {};

// 脚質ごとの基礎ポジション（0=先頭 1=最後方）とスパート挙動
// 東海Sの想定隊列（逃げ・番手・先行・好位・中団・後方）に合わせて6区分
const STYLE_BASE = { "逃げ": 0.02, "番手": 0.12, "先行": 0.24, "好位": 0.42, "中団": 0.62, "後方": 0.85 };
const STYLE_KICK = { "逃げ": 0.12, "番手": 0.22, "先行": 0.32, "好位": 0.55, "中団": 0.80, "後方": 1.05 }; // 終盤の伸び幅係数
const STYLE_FADE = { "逃げ": 0.32, "番手": 0.24, "先行": 0.16, "好位": 0.08, "中団": 0.03, "後方": 0.0 }; // 終盤の失速リスク

// 中京ダート1400mは「前傾ラップ＋直線の急坂」のタフなコースで、スピードだけでは押し切れず差しにもチャンスがあるとされる。
// 関屋記念のような明確な「前有利」の実績データは今回ないため、脚質バイアスは中立（1.0）とし、
// 個々の追い切り評価・展開のみで差がつくようにする。
const STYLE_BIAS = { "逃げ": 1.0, "番手": 1.0, "先行": 1.0, "好位": 1.0, "中団": 1.0, "後方": 1.0 };

// 東海ステークスの枠番バイアス実績データは今回ないため中立（1.0）とする。
const WAKU_BIAS = { 1: 1.0, 2: 1.0, 3: 1.0, 4: 1.0, 5: 1.0, 6: 1.0, 7: 1.0, 8: 1.0 };
...
const PHASES = [
  { key: "start", label: "芝スタート", from: 0.0, to: 0.12 },
  { key: "back", label: "3角まで（下り基調）", from: 0.12, to: 0.55 },
  { key: "corner", label: "コーナー", from: 0.55, to: 0.72 },
  { key: "straight", label: "直線・急坂（410.7m）", from: 0.72, to: 0.94 },
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 980, margin: "0 auto" }}>
        ...
        <header style={{ textAlign: "center", marginBottom: 22 }}>
          <div style={{ fontSize: 12, letterSpacing: 4, color: "#c9a34e", marginBottom: 6 }}>TOKAI STAKES SIMULATION</div>
          <h1 style={{
            fontSize: "clamp(28px, 5vw, 42px)", margin: 0, fontWeight: 900,
            letterSpacing: 1, color: "#f4f1e8", textShadow: "0 2px 12px rgba(0,0,0,.4)",
          }}>東海ステークス レースシミュレーション</h1>
          <div style={{ fontSize: 12.5, color: "#c9c2a4", marginTop: 8 }}>
            中京7R｜ダート1400m（左）｜3歳以上オープン・G3（別定）｜発走 15:35
          </div>
          <div style={{ fontSize: 13, color: "#a9c2ab", marginTop: 6 }}>
            直線410.7m／急坂1.8m（160mで上る）／3角まで600m（芝スタート）
          </div>
          <div style={{ fontSize: 11, color: "#6f8a76", marginTop: 4 }}>
            展開バイアス：想定隊列・追い切り総合評価を反映｜脚質・枠順の実績データは今回未反映（中立）｜天候は演出・参考情報のみ（着順計算には影響しません）
          </div>
        </header>
        ...
        {weather === "rain" && (
          <div style={{ marginTop: 20 }}>
            ...
            {Object.keys(RAIN_NOTES).length === 0 ? (
              <div style={{ fontSize: 12, color: "#8aa090", padding: "10px 12px", background: "#0e1e2a", border: "1px solid #234158", borderRadius: 10 }}>
                東海ステークスの道悪評価材料は今回まだ整理できていません。天候の演出のみご覧いただけます。
              </div>
            ) : (
            ...
            <div style={{ fontSize: 10, color: "#6f8a76", marginTop: 10, lineHeight: 1.6 }}>
              出典：東海ステークス2026 追い切り評価・想定隊列（画像提供分）。道悪評価は未整理のため参考情報はありません。
            </div>
          </div>
        )}
        ...
        <details style={{ marginTop: 20 }}>
          ...
          <div style={{ fontSize: 10, color: "#6f8a76", marginTop: 8 }}>
            出典：東海ステークス(G3) 追い切り 短評表（時計・過程・動き・総合評価の5段階、S＞A＞B＞C＞D）
          </div>
        </details>
        ...
        <details style={{ marginTop: 20 }}>
          ...
          <div style={{ marginTop: 10, display: "grid", gap: 6 }}>
            {[
              { label: "想定ペース", value: "M（ミドル）45% ／ H（ハイ）45% ／ S（スロー）10%" },
              { label: "レースの流れ", value: "外枠の13・14が主導権争い。先行勢は無理せず好位で脚を温存する形が想定される" },
              { label: "4角射程圏の中心", value: "5 ドンインザムード／7 ダノンフィーゴ／8 ベルジュロネット／9 インユアパレス／11 メイショウハチロー" },
            ].map((row) => (
              <div key={row.label} style={{
                padding: "8px 10px", background: "#0e2419", borderRadius: 6,
                border: "1px solid #1e3a2a", fontSize: 12.5,
              }}>
                <div style={{ color: "#e8c766", fontWeight: 700, marginBottom: 3 }}>{row.label}</div>
                <div style={{ color: "#c9c2a4" }}>{row.value}</div>
              </div>
            ))}
          </div>
          <div style={{ fontSize: 10, color: "#6f8a76", marginTop: 8, lineHeight: 1.6 }}>
            出典：東海ステークス2026 想定隊列（枠順確定版）。隊列・ペースは枠順確定後の想定であり、スタートや当日の馬場・展開で変わる可能性があります。
            関屋記念のような「前有利」を裏付ける複数年の実績データは今回ないため、脚質による着順バイアスは中立にしています。
          </div>
        </details>
        ...
        <details style={{ marginTop: 20 }}>
          ...
          <div style={{ marginTop: 10, fontSize: 12.5, color: "#c9c2a4", lineHeight: 1.7 }}>
            <div style={{ background: "#0e2419", border: "1px solid #1e3a2a", borderRadius: 8, padding: "10px 12px" }}>
              中京競馬場・ダート1400m（左回り）。2コーナー奥の引き込み線からスタートし、最初の200m弱は芝の上を走るためスピードに乗りやすい。
              スタート直後は緩やかな上り、バックストレッチ半ばからは緩やかな下りとなり、そのまま3角・4角を回る。
              直線入口では一転して上り坂となり、約160mで1.8m上る急坂が待つ。ゴールまでの直線は410.7mで、ダートコースとして東京に次ぐ長さ。
              前半が速くなりやすい「前傾ラップ」になりやすく、スピードだけでは押し切れないタフなコースとされる。
            </div>
          </div>
          <div style={{ fontSize: 10, color: "#6f8a76", marginTop: 8, lineHeight: 1.6 }}>
            ※これはコース形状に関する客観情報で、本シミュレーションの展開バイアスには直接反映していません。
          </div>
        </details>

        <div style={{ marginTop: 20, fontSize: 11, color: "#6f8a76", textAlign: "center", lineHeight: 1.7 }}>
          ※本シミュレーションは想定隊列・追い切り評価に基づく乱数演算による展開シミュレーションであり、実際のレース結果を予想・保証するものではありません。脚質・枠順による有利不利の実績データは今回反映していません。馬券購入は自己責任でお願いします。
        </div>
      </div>
    </div>
  );
...
ReactDOM.createRoot(document.getElementById('root')).render(<TokaiS />);
