...
function renderDifficultyBadge(difficulty) {
  ...
  return `<span class="difficulty-badge ${cls}">${escapeHtml(difficulty)}</span>`;
}
  ...
  main.innerHTML = `
    ...
    <div class="race-hero">
      ...
      <div class="hero-badges">
        ...
        ${renderDifficultyBadge(race['予想難易度'])}
      </div>
...
async function renderAdminRaceDetail(race) {
  ...
  detailEl.innerHTML = `
    <div class="card">
      ...
      <div class="divider-label" style="margin-top:16px;">予想難易度（誰でも見えます）</div>
      <p class="form-hint">ホーム画面に表示されます。荒れ具合の目安を共有したいときに設定してください。</p>
      <div class="form-group mt-8">
        <select id="a-difficulty-select" class="form-input">
          <option value="">未設定</option>
          <option value="超固い" ${race['予想難易度'] === '超固い' ? 'selected' : ''}>超固い</option>
          <option value="普通" ${race['予想難易度'] === '普通' ? 'selected' : ''}>普通</option>
          <option value="荒れ" ${race['予想難易度'] === '荒れ' ? 'selected' : ''}>荒れ</option>
          <option value="大荒れ" ${race['予想難易度'] === '大荒れ' ? 'selected' : ''}>大荒れ</option>
        </select>
      </div>
      <button class="submit-btn" id="a-difficulty-btn">予想難易度を保存</button>
      <p id="a-difficulty-status" class="save-status"></p>
  ...
  document.getElementById('a-difficulty-btn').addEventListener('click', async () => {
    const btn = document.getElementById('a-difficulty-btn');
    const statusEl = document.getElementById('a-difficulty-status');
    const difficulty = document.getElementById('a-difficulty-select').value;
    btn.disabled = true;
    statusEl.textContent = '';
    try {
      await API.adminUpdateDifficulty({ adminCode: state.adminCode, raceId, difficulty });
      showToast('予想難易度を保存しました');
      statusEl.textContent = difficulty ? `✓ 「${difficulty}」に設定しました` : '✓ 未設定に戻しました';
    } catch (err) {
      showToast(err.message, true);
    } finally {
      btn.disabled = false;
